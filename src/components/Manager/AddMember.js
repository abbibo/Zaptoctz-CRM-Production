...
const AddMember = () => {
  ...
  const refreshMembers = async () => {
    try {
      ...
      // Filter members to only show those assigned to this manager
      const managerData = allMembers.find((m) => m.id === managerId);
      if (managerData && managerData.assignedMembers) {
        const assigned = allMembers.filter((m) => managerData.assignedMembers.includes(m.id));
        setMembers(assigned);
      } else {
        setMembers([]);
      }
